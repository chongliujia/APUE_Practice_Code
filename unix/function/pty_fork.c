        ...
        const struct winsize *slave_winsize)
{
    ...
    if ((fdm = ptym_open(pts_name, sizeof(pts_name))) < 0) err_sys("can't open master pty: %s, error %d", pts_name, fdm);
    ...
    if ((pid = fork()) < 0) return (-1);
        ...
        if (slave_termios != NULL) {
            ...
            if (dup2(fds, STDOUT_FILENO) != STDOUT_FILENO) err_sys("dup2 error to stdout");
